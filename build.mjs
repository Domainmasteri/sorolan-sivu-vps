...
function injectLanguageOverlay(content, relativePath, locale) {
  ...
  const overlay = `\n    <nav class="language-overlay" aria-label="${escapeHtml(overlayLabel)}" id="lang-overlay" title="${escapeHtml(locale === 'fi' ? 'Siirrä kielen vaihtaja' : 'Drag to move')}">\n        <span class="language-overlay__label language-overlay__handle">${escapeHtml(overlayLabel)}</span>\n        <a class="language-overlay__link${locale === 'fi' ? ' is-active' : ''}" href="${fiHref}"${locale === 'fi' ? ' aria-current="page"' : ''}>🇫🇮</a>\n        <a class="language-overlay__link${locale === 'en' ? ' is-active' : ''}" href="${enHref}"${locale === 'en' ? ' aria-current="page"' : ''}>🇬🇧</a>\n    </nav>`;
