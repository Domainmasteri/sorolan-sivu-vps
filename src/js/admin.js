...
async function muokkaaLinkkia(domain, pathValue, currentUrl) {
  const newUrl = prompt(`Muokataan linkkiä /${pathValue}\nSyötä uusi kohdeosoite:`, currentUrl);
  if (!newUrl || newUrl === currentUrl) return;
  try {
    await jsonRequest('/api/links', {
      method: 'PUT',
      headers: authHeaders({ 'Content-Type': 'application/json' }),
      body: JSON.stringify({ domain, path: pathValue, newOriginalURL: newUrl })
    });
    await lataaTiedot();
  } catch (error) {
